...
if (cliArgs[0] == ("settings")) {
  if (cliArgs[1].startsWith("hardmode")) {
    const user = JSON.parse(fs.readFileSync('json/user.json','utf-8'));
    if (cliArgs[1].split(":")[1] == "true") {
      console.log("Hardmode: Enabled");
      user.game.hardmode = true;
      fs.writeFileSync('user.json', JSON.stringify(user));
      process.exit();
    } else if (cliArgs[1].split(":")[1] == "false") {
      console.log("Hardmode: Disabled");
      user.game.hardmode = true;
      fs.writeFileSync('user.json', JSON.stringify(user));
      process.exit(0);
    } else {
      console.log(cliArgs[1].split(":")[1] + "is invalid for " + cliArgs[1].split(":")[0]);
      process.exit(0);
    }
  }
}
...
let user = JSON.parse(fs.readFileSync('json/user.json','utf-8'));
